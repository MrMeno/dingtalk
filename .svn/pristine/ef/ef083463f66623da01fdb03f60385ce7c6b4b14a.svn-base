<template>
	<div class='container-fluid' style='padding-bottom:20px;'>
		<div class="row">
			<div class="col col-md-12 col-sm-12 text-center">
         <div class="row" style='padding-top:10px'>
                <div class="col col-md-12 col-sm-12 text-left" style='padding:0px'>
                    工具><span style='color:#999;font-size:12px'>行号查询</span>
                </div>
              </div>
                 <div class="row"  style='padding-top:50px;background-color:white;min-height:800px;line-height:30px;'>
                        <div class="col col-md-12 col-sm-12">
                          <div class="row" style='padding-top:50px'>
                            <div class="col col-md-4 col-sm-4 text-right">
                              银行名称
                            </div>
                            <div class="col col-md-4 col-sm-4">
                              <el-input v-model="name" placeholder="请输入银行名称"></el-input>
                            </div>
                          </div>

                          <div class="row" style='padding-top:20px'>
                            <div class="col col-md-4 col-sm-4 text-right">
                              大额行号
                            </div>
                            <div class="col col-md-4 col-sm-4">
                              <el-input v-model="num" placeholder="请输入12位行号"></el-input>
                            </div>
                          </div>

                          <div class="row" style='padding-top:20px'>
                            <div class="col col-md-6 col-sm-6 text-right">
                              <el-button :plain="true" @click='resets'>重置</el-button>
                            </div>
                            <div class="col col-md-6 col-sm-6 text-left">
                              <el-button type="primary" @click='getData()'>查询</el-button>
                            </div>
                          </div>

                           <div class="row" style="background-color:white;padding-top:50px" v-if='data.length==0'>
                           <div class="col col-md-6 col-sm-6 col-md-offset-3 col-sm-offset-3 text-center">
                               <img src="../../assets/blankspace.png" alt="">
                               <p style='color:#ddd'> 暂无数据</p>
                           </div>
                         </div>

                        <div class="row" style="margin:top:10px;background-color:white">
                          <div class="col col-md-6 col-sm-6 col-md-offset-3 col-ms-offset-3"  >
                              <div class="row"  v-for='items in data' style='background-color:#f4f5f7;margin-top:20px'>
                                <div class="col col-md-12">
                                   <div class="row">
                                <div class="col col-md-5 col-sm-5 text-center">
                                  银行名称
                                </div>
                                <div class="col col-md-7 col-sm-7 text-left">
                                   {{ items.bankName||'暂无'}}
                                </div>
                              </div>
                              <div class="row">
                                <div class="col col-md-5 col-sm-5 text-center">
                                  行号
                                </div>
                                <div class="col col-md-7 col-sm-7 text-left">
                                  {{items.bankNo||'暂无'}}
                                </div>
                              </div>
                              <div class="row">
                                <div class="col col-md-5 col-sm-5 text-center">
                                  电话
                                </div>
                                <div class="col col-md-7 col-sm-7 text-left">
                                  {{ items.bankContact||'暂无'}}
                                </div>
                              </div>
                              <div class="row">
                                <div class="col col-md-5 col-sm-5 text-center">
                                  地址
                                </div>
                                <div class="col col-md-7 col-sm-7 text-left">
                                  {{ items.bankAddr||'暂无'}}
                                </div>
                              </div>
                                </div>
                              </div>
                          </div>
                        </div>
                        </div>
                        </div>

			</div>
		</div>
	</div>
</template>
<script>
import {checkNum} from '../../lib/tools.js'
  export default {
    data() {
      return {
        activeName: 'first',
        name:'',
        num:'',
        pageSize:10,
        pageNo:1,
        data:[]
      };
    },
    mounted(){
    },
    methods: {
     resets(){
      this.name='';
      this.num='';
     }, 
     check(name,num){
      var result={error:'',flag:false};
      if(name==''&&num==''){
          result.error='请输入查询参数';
          result.flag=false;
         }
         else if(name!=''&&num==''){
          result.flag=true
         }
        else if(name==''&&num!=''){
          if(checkNum(num)&&num.length==12){
             result.flag=true
          }
          else{
            result.error='请输入12位行号';
            result.flag=false;
          }
         }
     else if(name!=''&&num!=''){
           if(checkNum(num)&&num.length==12){
             result.flag=true
          }
          else{
            result.error='请输入12位行号';
            result.flag=false;
          }

       }
      return result
     },
      getData(){
        let self=this;
       var params='?bankNo='+self.num;
        params+='&bankName='+self.name;
        params+='&pageNo='+self.pageNo;
         params+='&pageSize='+self.pageSize;
         var checked=self.check(this.name,this.num);
         if(checked.flag)
         {
          this.$http.get('/dingtalk/tools/get/bank/no'+params).then(res=>{
         if(res.data.code=="000000"){
         
          self.data=res.data.data.result;
           console.log(self.data)
         }
         else{
             self.$notify.error({
          title: '错误',
          message: '查询失败'
        });
         }
        })
          }
           else{
             self.$notify.warning({
            title: '警告',
             message: checked.error
             });

                 }
   
      }
    }
  };
</script>